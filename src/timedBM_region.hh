                      ...
                      int &hashCalcCount)
{
  ...
  // precomputation
  {
    ...
    // make R(A)
    printDuration(ta_to_ra(A,RA), "ta_to_ra: ");
    //! L = L'    
    std::vector< std::vector<RAState> > L;
    //! Ls = L'_s
    std::vector< std::vector<RAState> > Ls;
    printDuration(calcL (RA,L,m), "calcL: ");
    ...
    nsec = std::chrono::duration_cast<std::chrono::nanoseconds>(dur).count();
       std::cout << std::scientific << "R'(A2): " << nsec / 1000000.0 << " ms" << std::endl;
#endif

    PartialRunChecker<Region> isPartialRun2(RA2,RA,A.edges.size());
    ...
    for (TAState s = 0; s < A.edges.size();s++) {
      hashCalcCount++;
      int ms;
      printDuration(calcLs (RA,Ls,s,m,ms), "calcL" << s << ": ");
      int n;
      start = std::chrono::system_clock::now();
      for (n = 1;n < m-1; n++) {
